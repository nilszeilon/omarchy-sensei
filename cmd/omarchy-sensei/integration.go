...
func senseiLua() string {
  ...
  function hl.bind(keys, dispatcher, options)
    ...
    if not description or key_text:find("mouse", 1, true) or key_text:find("switch:", 1, true) then
...
func menuOverrideBlock() string {
	...
	for _, action := range coachedActions {
		...
		if action.MenuID == "trigger.reminder.set" {
			fmt.Fprintf(&out, "  %s: {\"aliases\":[\"reminder-set\",\"remind\"],\"action\":%s},\n", encodedID, encodedCommand)
		} else {
