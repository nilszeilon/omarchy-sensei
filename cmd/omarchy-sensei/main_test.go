...
func TestSlowActionOpensTask(t *testing.T) {
	...
	result := snapshotAfter([]Observation{{
		ObservedAt: now,
		Action:     "focus-window",
		Title:      "Focus window",
		Trigger:    "mouse",
	...
	if len(result.Tasks[0].Shortcuts) != 1 || result.Tasks[0].Shortcuts[0] != "SUPER + LEFT/RIGHT" {
...
func TestMatchingShortcutClosesTask(t *testing.T) {
	...
	observations := []Observation{
		{ObservedAt: now, Action: "browser", Title: "Open browser", Trigger: "menu", Shortcut: "SUPER + B"},
		{ObservedAt: now.Add(time.Second), Action: "browser", Title: "Open browser", Trigger: "shortcut"},
	}

	if tasks := snapshotAfter(observations, now).Tasks; len(tasks) != 0 {
...
func TestSlowActionAfterShortcutReopensTask(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "browser", Title: "Open browser", Trigger: "shortcut"},
		{ObservedAt: now.Add(time.Second), Action: "browser", Title: "Open browser", Trigger: "menu", Shortcut: "SUPER + B"},
	}

	if tasks := snapshotAfter(observations, now).Tasks; len(tasks) != 1 {
...
func TestMenuRoutedByShortcutDoesNotReopenTask(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "set-reminder", Title: "Set reminder", Trigger: "menu", Shortcut: "SUPER CTRL + R"},
		{ObservedAt: now.Add(time.Second), Action: "set-reminder", Title: "Set reminder", Trigger: "shortcut"},
		{ObservedAt: now.Add(1200 * time.Millisecond), Action: "set-reminder", Title: "Set reminder", Trigger: "menu", Shortcut: "SUPER CTRL + R"},
	}
...
func TestRepeatedSlowUsesStayOneTask(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "screenshot", Title: "Screenshot", Trigger: "menu", Shortcut: "PRINT"},
		{ObservedAt: now.Add(time.Second), Action: "screenshot", Title: "Screenshot", Trigger: "mouse", Shortcut: "PRINT"},
	}

	tasks := snapshotAfter(observations, now).Tasks
...
func TestTasksAreOrderedByWorstOffender(t *testing.T) {
	...
	observations := []Observation{
		{ObservedAt: now, Action: "browser", Title: "Open browser", Trigger: "menu", Shortcut: "SUPER + B"},
		{ObservedAt: now.Add(time.Second), Action: "terminal", Title: "Open terminal", Trigger: "menu", Shortcut: "SUPER + RETURN"},
		{ObservedAt: now.Add(2 * time.Second), Action: "terminal", Title: "Open terminal", Trigger: "mouse", Shortcut: "SUPER + RETURN"},
		{ObservedAt: now.Add(3 * time.Second), Action: "terminal", Title: "Open terminal", Trigger: "menu", Shortcut: "SUPER + RETURN"},
	}
...
func TestTaskIncludesAllResolvedShortcuts(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "screenshot", Title: "Screenshot", Trigger: "shortcut"},
		{ObservedAt: now.Add(time.Second), Action: "screenshot", Title: "Screenshot", Trigger: "menu", Shortcut: "PRINT", Shortcuts: []string{"PRINT", "SUPER + P"}},
	}
...
func TestWorkspaceActionsCollapseIntoOneTask(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "switch-to-workspace-2", Title: "Switch to workspace 2", Trigger: "mouse", Shortcut: "SUPER + 2"},
		{ObservedAt: now.Add(time.Second), Action: "switch-to-workspace-5", Title: "Switch to workspace 5", Trigger: "mouse", Shortcut: "SUPER + 5"},
	}
	tasks := snapshotAfter(observations, now).Tasks
	...
	if len(tasks[0].Shortcuts) != 1 || tasks[0].Shortcuts[0] != "SUPER + TAB" {
...
func TestAnyWorkspaceShortcutClosesCollapsedTask(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	for _, shortcutObservation := range []Observation{
		{ObservedAt: now.Add(time.Second), Action: "next-workspace", Title: "Next workspace", Trigger: "shortcut"},
		{ObservedAt: now.Add(time.Second), Action: "switch-to-workspace-8", Title: "Switch to workspace 8", Trigger: "shortcut"},
	} {
		observations := []Observation{
			{ObservedAt: now, Action: "switch-to-workspace-3", Title: "Switch to workspace 3", Trigger: "mouse", Shortcut: "SUPER + 3"},
			shortcutObservation,
		}
		if tasks := snapshotAfter(observations, now).Tasks; len(tasks) != 0 {
			t.Fatalf("expected %q to close workspace group, got %#v", shortcutObservation.Title, tasks)
		}
	}
}
...
func TestPositionalPanelTasksCollapseButNamedPanelStaysIndependent(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "bar-panel-2", Title: "Bar panel 2", Trigger: "mouse", Shortcut: "SUPER CTRL + 2"},
		{ObservedAt: now.Add(time.Second), Action: "bar-panel-3", Title: "Bar panel 3", Trigger: "mouse", Shortcut: "SUPER CTRL + 3"},
		{ObservedAt: now.Add(2 * time.Second), Action: "bluetooth", Title: "Bluetooth", Trigger: "mouse", Shortcut: "SUPER CTRL + B"},
		{ObservedAt: now.Add(3 * time.Second), Action: "bar-panel-1", Title: "Bar panel 1", Trigger: "shortcut"},
	}
	state := stateAfter(observations)
	tasks := snapshotFromState(state, false).Tasks
	if len(tasks) != 1 || tasks[0].Action != "bluetooth" {
		t.Fatalf("expected generic panel key to leave named Bluetooth open, got %#v", tasks)
	}
	applyObservation(&state, Observation{ObservedAt: now.Add(4 * time.Second), Action: "bluetooth", Title: "Bluetooth", Trigger: "shortcut"})
	if tasks := snapshotFromState(state, false).Tasks; len(tasks) != 0 {
...
func TestShortcutTotalDeduplicatesOnePhysicalChord(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := []Observation{
		{ObservedAt: now, Action: "focus-next", Trigger: "shortcut"},
		{ObservedAt: now.Add(20 * time.Millisecond), Action: "reveal-active", Trigger: "shortcut"},
		{ObservedAt: now.Add(150 * time.Millisecond), Action: "focus-next", Trigger: "shortcut"},
		{ObservedAt: now.Add(160 * time.Millisecond), Action: "terminal", Trigger: "shortcut"},
	}
	if got := stateAfter(observations).TotalShortcuts; got != 2 {
		t.Fatalf("expected two physical shortcut uses, got %d", got)
	}
}

func TestSnapshotIncludesLifetimeLevel(t *testing.T) {
	now := time.Date(2026, 8, 21, 12, 0, 0, 0, time.Local)
	observations := make([]Observation, 10)
...
func snapshotAfter(observations []Observation, now time.Time) Snapshot {
	state := stateAfter(observations)
	pruneTransientState(&state, now)
	return snapshotFromState(state, false)
}
